...
extern long tgl_tris, tgl_culls, tgl_texloads, tgl_lm_updates;   // tgl_engine.cc per-frame counters
...
void GL_EndRendering(void) {
  ...
  // headless capture: GEN_FRAME_PPM_FILES dumps frameNNNN.ppm; HW_MAX_FRAMES=N exits
  {
    ...
    if (hb) fprintf(stderr, "[hb] frame %d: %ld tris, %ld culled, %ld texloads, %ld lm_updates\n", frame, tgl_tris, tgl_culls, tgl_texloads, tgl_lm_updates);
    tgl_tris = 0; tgl_culls = 0; tgl_texloads = 0; tgl_lm_updates = 0;
